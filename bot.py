...
# ---- INTENTS (message content REQUIRED for triggers) ----  
...
async def host(interaction: discord.Interaction, code: str):  
    ...
    user = active_lobbies.setdefault(  
        interaction.user.id, {"normal": None, "modded": None}  
    )  
...
async def on_message(message: discord.Message):  
    ...
    if any(trigger in content for trigger in triggers):  
        channels_text = " ".join(f"<#{cid}>" for cid in LOBBY_CHANNEL_IDS)  
  
        await message.channel.send(  
            f"🛸 We usually host at <t:1770060600:t>, "  
            f"but you can always check these channels for current lobbies: {channels_text}"  
        )  
